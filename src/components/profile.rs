...
pub fn render(username: &str, stats_value: &serde_json::Value) -> String {
    ...
    let mut uhc_kills = 0;
    let mut uhc_wins = 0;
    let mut uhc_losses = 0;
    let mut uhc_matches = 0;
    let mut uhc_assists = 0;
    ...
    if let Some(fields) = stats_value.get("fields") {
        if let Some(stats) = fields.get("stats").and_then(|s| s.get("mapValue")).and_then(|m| m.get("fields")) {
            ...
            // Parse UHC
            if let Some(uhc) = stats.get("uhc").and_then(|v| v.get("mapValue")).and_then(|m| m.get("fields")) {
                uhc_kills = get_json_long(uhc, "kills");
                uhc_wins = get_json_long(uhc, "wins");
                uhc_losses = get_json_long(uhc, "losses");
                uhc_matches = get_json_long(uhc, "matches_played");
                uhc_assists = get_json_long(uhc, "assists");
            }
        }
    }
    
    let total_kills = ttr_kills + uhc_kills;
    let total_wins = ttr_wins + uhc_wins;
    let total_losses = ttr_losses + uhc_losses;
    let total_matches = ttr_matches + uhc_matches;
...
<div class="profile-container">
  ...
  <div class="events-card glass-surface mt-5 slide-up" style="animation-delay: 0.1s;">
    ...
    <div class="events-timeline">
      ...
      <div class="event-item glass-surface">
        <div class="event-meta">
          <span class="event-badge gold" style="background: rgba(231, 76, 60, 0.15); color: #e74c3c; border-color: rgba(231, 76, 60, 0.2);">UHC</span>
        </div>
        <div class="event-details">
          <h3>Ultra Hardcore (UHC)</h3>
          <p>Supervivencia extrema sin regeneración de vida natural. Estadísticas de combate y supervivencia.</p>
          <div class="stats-details-grid">
            <div class="stat-detail-card glass-surface">
              <span class="material-symbols-rounded stat-icon color-kills">swords</span>
              <div class="stat-detail-info">
...
"##,
        ...
        uhc_kills, uhc_assists, uhc_wins, uhc_losses, uhc_matches
    )
}
