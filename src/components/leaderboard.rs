pub fn render(data: &serde_json::Value, active_game: &str) -> String {
...
<td style="color: #60a5fa; font-weight: 600;">{}</td>
    ...
    let ttr_active = if active_game == "ttr" { "active" } else { "" };
    let uhc_active = if active_game == "uhc" { "active" } else { "" };
    let subtitle = if active_game == "uhc" { "Ultra Hardcore UHC • Actualizado en vivo" } else { "The Towers Remastered • Actualizado en vivo" };
...
<p class="subtitle slide-up">{}</p>
</div>

<div class="tabs-scroll mt-5 slide-up" style="display: flex; gap: 10px; justify-content: center;">
<a class="btn btn-glass {}" href="/leaderboard?game=ttr" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px;"><span class="material-symbols-rounded">fort</span> The Towers</a>
<a class="btn btn-glass {}" href="/leaderboard?game=uhc" style="text-decoration: none; display: inline-flex; align-items: center; gap: 8px;"><span class="material-symbols-rounded">swords</span> UHC</a>
</div>
